...
export default function Programmers() {
  ...
  const [progs, setProgs] = useState(listOfAwesome);
  const [featProg, setFeatProg] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    // let featName;
    // progs.forEach((prog) =>
    //   prog.id === featProg ? (featName = prog.name) : null
    // );
    // return featName;
    return progs.find((prog) => prog.id === featProg).name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: featProg ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
